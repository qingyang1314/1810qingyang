  ...
  <body>
    <div class="maincont">
     ...
     @foreach($data as $key=>$val)
      <dl>
       <dt><a href="/proinfo{{$val->goods_id}}">
        <img src="http://upload.tp5.com/{{$val->goods_img}}" width="100" height="100" /></a></dt>
       <dd>
        <h3><a href="/proinfo{{$val->goods_id}}">{{$val->goods_name}}</a></h3>
        <div class="prolist-price"><strong>¥{{$val->shop_price}}</strong> <span>¥原价{{$val->shop_price}}</span></div>
        <div class="prolist-yishou"> <em>库存：{{$val->goods_number}}</em></div>
       </dd>
